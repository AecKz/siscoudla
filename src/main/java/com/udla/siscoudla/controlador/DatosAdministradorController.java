 ...
 */
@WebServlet("/DatosAdministradorController")
public class DatosAdministradorController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public DatosAdministradorController() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String generoAdministrador = request.getParameter("genero") == null ? ""
				: request.getParameter("genero");
		String fechaNacimientoAdministrador = request.getParameter("fechaNacimiento") == null ? ""
				: request.getParameter("fechaNacimiento");
		String telefonoAdministrador = request.getParameter("telefono") == null ? ""
		...
		try {
			...
			if (tipoConsulta.equals("guardarDatosProfile")) {
				if (Utilitarios.verificarRolAdministrador(valorUsuario)) {
					...
					String fecha = Utilitarios.fechaDatePickertoDate(fechaNacimientoAdministrador);
					Date fechaNacimientoPersona = Utilitarios.stringToDate(fecha);									
					personaVO.setFechaNacimiento(fechaNacimientoPersona);
					personaVO.setGenero(generoAdministrador);
					personaVO.setTelefono(telefonoAdministrador);
