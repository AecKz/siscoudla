 ...
 */
	...
	$(document).ready(function() {
		...
		$("#dataTableContent").append("<tr><td colspan='2'>Seleccione un Horario</td></tr>");
			// Datos Iniciales
			...
			$.ajax({
				...
				success : function(data) {
                    ...
                    $.each(listadoHorariosCombos, function (index) {
                        $("#selectHorario").append("<option value='" + listadoHorariosCombos[index].codigo + "'>" + listadoHorariosCombos[index].horario + "</option>");
                    });				
	...
	function tomarLista(){
	$("#loading").show();
	var codigo = $('#selectHorario').val();
	//Cargar Turnos
	$.ajax({
		url : '../TomarListaController',
		data : {
			"codigo":codigo,
			"tipoConsulta" : "tomarLista"
		},
		type : 'POST',
		datatype : 'json',
		success : function(data) {
